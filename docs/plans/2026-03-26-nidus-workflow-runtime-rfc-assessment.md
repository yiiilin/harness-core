...
- [ ] Publish a maintainer decision that `harness-core` stays on the current `session + plan + step` architecture for this wave.
- [ ] Update `docs/EMBEDDER_VNEXT.md` and `docs/EMBEDDER_VNEXT_REALITY_CHECK.md` with an explicit “not a full workflow graph runtime yet” statement.
- [ ] Define acceptance language for “program-runtime extensions” versus “new workflow IR”, so follow-up PRs do not smuggle in graph-engine concepts piecemeal.

### P1: Extend Approval To Cover Request-Level Workflow Gating

- [ ] Add a session/request-level approval gate API that blocks execution before the first runnable program/plan step.
- [ ] Keep resume semantics pointed at the original blocked execution context instead of creating a fresh session/runtime path.
- [ ] Decide whether second confirmation belongs in `approval.Record` or should remain a generic `BlockedRuntime` condition; implement only one model, not both.
- [ ] Add restart/recovery tests for request-level approval, mid-program approval, rejection, and repeated resume attempts.

### P2: Make Interactive Handles Program-Native

- [ ] Add a typed runtime-handle reference contract that later program steps can consume without scraping opaque metadata.
- [ ] Extend program binding resolution so later steps can address runtime handles directly, not only structured output/artifact refs.
- [ ] Add native program operations for interactive handle lifecycle: start, inspect/view, write, verify, close.
- [ ] Preserve stable handle identity, cycle linkage, and version-safe updates through replay and restart.
- [ ] Add approval-gated interactive-program tests so handle operations can sit behind approval boundaries without identity loss.

### P3: Strengthen The Existing Program Runtime Instead Of Replacing It

- [ ] Add dependency-aware scheduling for ready sibling `ProgramNode`s beyond target fan-out groups.
- [ ] Introduce explicit per-program or per-node concurrency policy instead of relying only on target-fanout aggregate metadata.
- [ ] Keep typed dataflow on the current `ProgramInputBinding` model and extend it only where the missing reference kind is kernel-owned.
- [ ] Ensure shell, tool, artifact-producing, and interactive-handle operations can coexist in one `Program`.
- [ ] Add replay/projection views that expose program lineage clearly enough for restart hydration without inventing a full graph-event store yet.

### P4: Harden Projection And Recovery Around Newly Accepted Features

- [ ] Add stable projection fields for program node lineage, approval linkage, blocked-runtime linkage, and runtime-handle lineage.
- [ ] Add idempotent recovery tests for concurrent ready-node execution so restart does not duplicate sibling work.
- [ ] Add idempotent recovery tests for handle close, handle invalidation, and approval replay.
- [ ] Add sequence/causality assertions for any new audit event types introduced by request-level approval or program-native interactive operations.

### P5: Separate Design Track For A Future Workflow Graph Kernel

- [ ] Write a dedicated design doc for a real workflow IR before implementing any `Scope`, `Edge`, `AppendPatch`, or descendant-mutation API.
- [ ] Decide whether that future design replaces `Plan` execution, lowers into `Plan`, or runs beside it.
- [ ] Define mutation-safety rules, failure-propagation rules, and replay guarantees before accepting any graph append API.
- [ ] Require at least one concrete migration story from current `Program` execution into the future graph model before building the store/scheduler.
