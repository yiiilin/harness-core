...
func capabilityErrorResult(spec action.Spec, err error) action.Result {
	...
	case errors.Is(err, capability.ErrCapabilityViewNotFound):
		return action.Result{OK: false, Error: &action.Error{Code: "CAPABILITY_VIEW_NOT_FOUND", Message: spec.ToolName}}
	case errors.Is(err, capability.ErrCapabilityViewDrift):
		return action.Result{OK: false, Error: &action.Error{Code: "CAPABILITY_VIEW_DRIFT", Message: spec.ToolName}}
	default:
