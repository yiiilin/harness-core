...
`)).WillReturnResult(sqlmock.NewResult(0, 1))
	err = repo.Emit(audit.Event{Type: audit.EventToolCalled, SessionID: "sess1", TaskID: "task1", StepID: "step1", AttemptID: "attempt1", ActionID: "action1", TraceID: "trace1", CausationID: "attempt1", Payload: map[string]any{"tool_name": "shell.exec"}, CreatedAt: 1})
	if err != nil {
		t.Fatalf("emit: %v", err)
	}

	listRows := sqlmock.NewRows([]string{"event_id", "type", "session_id", "task_id", "step_id", "attempt_id", "action_id", "trace_id", "causation_id", "payload_json", "created_at"}).
		AddRow("evt1", "tool.called", "sess1", "task1", "step1", "attempt1", "action1", "trace1", "attempt1", `{"tool_name":"shell.exec"}`, int64(1))
...
ORDER BY created_at ASC`)).WithArgs("sess1").WillReturnRows(listRows)
	...
	allRows := sqlmock.NewRows([]string{"event_id", "type", "session_id", "task_id", "step_id", "attempt_id", "action_id", "trace_id", "causation_id", "payload_json", "created_at"}).
		AddRow("evt1", "tool.called", "sess1", "task1", "step1", "attempt1", "action1", "trace1", "attempt1", `{"tool_name":"shell.exec"}`, int64(1)).
		AddRow("evt2", "verify.completed", "sess1", "task1", "step1", "attempt1", "action1", "trace1", "action1", `{"success":true}`, int64(2))
	mock.ExpectQuery(regexp.QuoteMeta(`
SELECT event_id, type, session_id, task_id, step_id, attempt_id, action_id, trace_id, causation_id, payload_json, created_at
FROM audit_events
