...
func TestSessionRepoCreateGetUpdateList(t *testing.T) {
	...
	createRows := sqlmock.NewRows([]string{"session_id", "task_id", "parent_session_id", "title", "goal", "phase", "current_step_id", "summary", "retry_count", "metadata_json", "created_at", "updated_at"}).
		AddRow("sess1", nil, nil, "demo", "goal", "received", nil, nil, 0, "{}", int64(1), int64(1))
...
`)).WillReturnRows(createRows)
	...
	getRows := sqlmock.NewRows([]string{"session_id", "task_id", "parent_session_id", "title", "goal", "phase", "current_step_id", "summary", "retry_count", "metadata_json", "created_at", "updated_at"}).
		AddRow("sess1", "task1", nil, "demo", "goal", "plan", nil, nil, 1, "{}", int64(1), int64(2))
	mock.ExpectQuery(regexp.QuoteMeta(`
...
`)).WillReturnResult(sqlmock.NewResult(0, 1))
	...
	listRows := sqlmock.NewRows([]string{"session_id", "task_id", "parent_session_id", "title", "goal", "phase", "current_step_id", "summary", "retry_count", "metadata_json", "created_at", "updated_at"}).
		AddRow("sess1", "task1", nil, "demo", "goal", "plan", nil, "done", 1, "{}", int64(1), int64(3))
	mock.ExpectQuery(regexp.QuoteMeta(`
SELECT session_id, task_id, parent_session_id, title, goal, phase, current_step_id, summary, retry_count, metadata_json, created_at, updated_at
FROM sessions
