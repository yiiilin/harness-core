...
func TestRepositoryFactoryFromTx(t *testing.T) {
	...
	factory := postgres.RepositoryFactory{
		...
		PlanningFactory: func(db postgres.DBTX) planning.Store {
			return planningrepo.New(db)
		},
		AuditFactory: func(db postgres.DBTX) audit.Store { return auditrepo.New(db) },
	}
	repos := factory.FromTx(tx)
	if repos.Sessions == nil || repos.Tasks == nil || repos.Plans == nil || repos.PlanningRecords == nil || repos.Audits == nil {
